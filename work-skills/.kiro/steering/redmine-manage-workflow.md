...
凡是通过 AI 写入 Redmine 的文本内容，**必须**在文本末尾追加标记 `【AI Generate】`。

**适用范围：**
- `create_issue` / `update_issue` 的 `subject`（主题）— 无论是用户提供还是 AI 拟定，一律追加标记
- `create_issue` 的 `description`（任务描述）
- `update_issue` 的 `notes`（更新备注）

**格式：** 在文本内容末尾换行追加 `【AI Generate】`

**示例：**

更新备注：
```
目标Sprint由 平台Sprint2026-03 更新为 平台Sprint2026-04
【AI Generate】
```

任务描述：
```
中节能甘肃集控录波读取驱动文件僵死（03-20）——0.5
【AI Generate】
```

**规则：**
1. 每次调用 `update_issue` 时，`notes` 不得为空
2. 标记 `【AI Generate】` 放在文本最后一行，与正文之间换行分隔
3. 用户明确提供的原文内容保持不变，仅在末尾追加标记
...
- 图片保存路径: `docs/redmine_summary_<YYYYMMDD>.png`
- 图表标题包含项目名、月份、查询类型信息
