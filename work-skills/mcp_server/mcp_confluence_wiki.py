...
# @AI_GENERATED: Kiro v1.0
def _render_mermaid_to_png(mermaid_code: str, output_path: str) -> bool:
    ...
    try:
        with tempfile.NamedTemporaryFile(mode='w', suffix='.mmd', delete=False, encoding='utf-8') as f:
            ...
            mmd_path = f.name
        # Try mmdc from common global npm paths on Windows
        mmdc_cmd = 'mmdc'
        npm_global = os.path.join(os.environ.get('APPDATA', ''), 'npm', 'mmdc.cmd')
        if os.path.isfile(npm_global):
            mmdc_cmd = npm_global
        result = subprocess.run(
            [mmdc_cmd, '-i', mmd_path, '-o', output_path, '-b', 'white', '-s', '2'],
...
def _md_to_confluence_html(md_text: str, uploaded_images: set) -> tuple:
    """Markdown 全文转 Confluence storage HTML。返回 (html, pending_mermaids)。"""
    lines = md_text.split("\n")
    parts = []
    pending_mermaids = []
    i = 0
    ...
    while i < len(lines):
        ...
        # @AI_GENERATED: Kiro v1.0
        # details/summary -> Confluence expand macro
        if line.strip() == '<details>':
            ...
            while i < len(lines) and lines[i].strip() == '':
                i += 1
            if i < len(lines):
                sm = re.match(r'<summary>(.*?)</summary>', lines[i].strip())
                if sm:
                    expand_title = sm.group(1).strip()
                    i += 1
            # collect body until </details>
            expand_body_lines = []
            while i < len(lines) and lines[i].strip() != '</details>':
                expand_body_lines.append(lines[i])
                i += 1
            if i < len(lines):
                i += 1  # skip </details>
            # convert inner content
            inner_html, inner_mermaids = _md_to_confluence_html(
                "\n".join(expand_body_lines), uploaded_images)
            pending_mermaids.extend(inner_mermaids)
            parts.append(
                f'<ac:structured-macro ac:name="expand">'
                f'<ac:parameter ac:name="title">{_xml_escape(expand_title)}</ac:parameter>'
        ...
        if line.strip().startswith("```") and in_code:
            in_code = False
            code_body = chr(10).join(code_lines)
            if code_lang == "mermaid":
                # @AI_GENERATED: Kiro v1.0
                img_name = f"mermaid_{len(pending_mermaids) + 1:03d}.png"
                pending_mermaids.append((img_name, code_body))
                parts.append(
                    f'<p><ac:image ac:width="800">'
                    f'<ri:attachment ri:filename="{img_name}"/>'
                    f'</ac:image></p>'
                )
                # @AI_GENERATED: end
            else:
...
def upload_markdown_to_page(page_id: str | int, md_file: str, img_dir: str = "") -> str:
    ...
    # 转换 Markdown 到 Confluence HTML（含 mermaid 待渲染列表）
    html_content, pending_mermaids = _md_to_confluence_html(body_md, uploaded_images)

    # @AI_GENERATED: Kiro v1.0
    # Render and upload mermaid images
    import tempfile
    if pending_mermaids:
        tmp_dir = tempfile.mkdtemp(prefix='mermaid_')
        for img_name, mermaid_code in pending_mermaids:
            png_path = os.path.join(tmp_dir, img_name)
            if _render_mermaid_to_png(mermaid_code, png_path):
                result = _upload_attachment(page_id, png_path)
                if result:
                    uploaded_images.add(result)
        import shutil
    ...
    return json.dumps({
        ...
        "mermaid_found": len(pending_mermaids),
        "message": "页面更新成功"
    }, ensure_ascii=False, indent=2)
